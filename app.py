...
st.markdown('<b>The US has a mass incarceration problem.</b> According to the [American Civil Liberties Union (ACLU)](https://www.aclu.org/issues/smart-justice/mass-incarceration), "despite making up close to 5% of the global population, the U.S. has nearly 25% of the world\'s prison population". And over the course of the COVID-19 pandemic, many state and local governments have failed to take adequate measures to protect prisoners from the disease. Prisoners live in cramped, crowded, and often unsanitary facilities, and interact with staff who travel in-and-out of the surrounding community.', unsafe_allow_html = True)
st.write('I wanted to make this data app to explore, state-by-state, the impact of COVID-19 in prisons versus the overall population.')
